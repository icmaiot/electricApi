...
const Lineaprod = require('../models').Lineaprod;
...
module.exports = {
    get: async function (req, res) {
        try {
            ...
            let response = await Lineaprod.findAll({
                attributes: ['idlineaprod', 'lineaprod', 'Descripcion'],
                where: query,
            })
            if (response) {
                res.status(200).send({
    ...
    create: async function (req, res) {
        try {
            ...
            let new_perfil = new Lineaprod(req.body);
    ...
    delete: async function (req, res) {
        try {
            const response = await Lineaprod.destroy({
                where: { idlineaprod: req.params.id }
            })
    ...
    update: async function (req, res) {
        try {
            const resp = await Lineaprod.update(req.body, {
                where: { idlineaprod: req.params.id }
            })
    ...
    read: async function (req, res) {
        try {
            let perfilConfig = await Lineaprod.findOne({ where: { idperfil: req.params.id } });
            if (perfilConfig) {
