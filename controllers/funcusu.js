...
const Funcusu = require('../models').Funcusu;
...
module.exports = {
    get: async function (req, res) {
        try {
            let response = await Funcusu.findAll({
                attributes: ['IDfuncusu', 'funcusu']
            })
            if (response) {
                res.status(200).send({
                    code: 200, response
    ...
    create: async function (req, res) {
        try {
            let response_new = new Funcusu(req.body);
    ...
    delete: async function (req, res) {
        try {
            const response = await Funcusu.destroy({
                where: { IDfuncusu: req.params.id }
            })
    ...
    update: async function (req, res) {
        try {
            const resp = await Funcusu.update(req.body, {
                where: { IDfuncusu: req.params.id }
            })
    ...
    read: async function (req, res) {
        try {
            let response = await Funcusu.findOne({ where: { IDfuncusu: req.params.id } });
            if (response) {
