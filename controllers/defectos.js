...
const Defectos = require('../models').Defectos;
...
module.exports = {
    get: async function (req, res) {
        try {
            ...
            let response = await Defectos.findAll({
                attributes: ['id', 'cantdef', 'lotedef', 'iddefreg', 'idusuarios', 'idusuarioreg'],
                where: query,
                include: [{
    ...
    getDefectosins: async function (req, res) {
        try {
            let tname = req.query.tname == '' ? '-1' : req.query.tname;
            let lote = req.query.lote == '' ? '-1' : req.query.lote;
            const response = await _sequelize.query('CALL defectosins(:tname,:lote)', { replacements: { tname: tname, lote: lote } });
            if (response) {
                res.status(200).send({ code: 200, response });
            } else {
                throw new Error(CONST_ERROR.NOT_FOUND)
    ...
    create: async function (req, res) {
        try {
            let response_new = new Defectos(req.body);
    ...
    delete: async function (req, res) {
        try {
            const response = await Defectos.destroy({
    ...
    update: async function (req, res) {
        try {
            const resp = await Defectos.update(req.body, {
                where: { id: req.params.id }
            })
