...
const Controlprod = require('../models').Controlprod;
...
module.exports = {
    get: async function (req, res) {
        try {
            ...
            let response = await Controlprod.findAll({
                attributes: ['idcontrolprod','skuprod','mts','mtsmin','skunext','statusprod'],
                where: query,
            })
            if (response) {
                res.status(200).send({
    ...
    create: async function (req, res) {
        try {
            let response_new = new Controlprod(req.body);
    ...
    delete: async function (req, res) {
        try {
            const response = await Controlprod.destroy({
                where: { idcontrolprod: req.params.id }
            })
    ...
    update: async function (req, res) {
        try {
            const resp = await Controlprod.update(req.body, {
                where: { idcontrolprod: req.params.id }
            })
