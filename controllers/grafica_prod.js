...
const Grafico_prod = require('../models').Grafico_prod;
...
module.exports = {
    getp: async function (req, res) {
        try{
            let response = await Grafico_prod.findAll();
            if(response){
    ...
    getConsulp: async function (req, res){
        let maq = req.query.maq;
        let r1 = req.query.rango1;
        let r2 = req.query.rango2;
        try{
            const response = await _sequelize.query('CALL grafo_prod( :maq, :r1, :r2)', 
    ...
    getMaqp: async function (req, res) {
      try { 
        let response = await Grafico_prod.findAll({
          /// distinct 
          attributes: ['maquina'],
    ...
    getCausap: async function (req, res){
      ...
      try{
        const response = await _sequelize.query('CALL causa_prod(:causa, :ma, :r1, :r2)', 
